...
string soibean::usage(const std::string& cwdProg) const {
...
std::vector<int> soibean::generateRandomNumbers(const int maxNum, const int k) {
    ...
    std::mt19937 gen(rd()); 
    int min_value = 0;    
    int max_value = static_cast<int>(maxNum) - 1;  // Adjusted to prevent out-of-range
    std::uniform_int_distribution<> distrib(min_value, max_value);

    int n = static_cast<int>(k);  // Number of random numbers to generate
    for(int i = 0; i < n; ++i) {
        int random_number = static_cast<int>(distrib(gen));
