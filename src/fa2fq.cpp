...
void Trailmix::write_fq_read(auto & dummyFASTQFile, int offset, const int window_size, const string &fastaseq, char dummyqualscore) {
        string seq_to_write, qual_to_write;
        for (const char base : fastaseq.substr(min(offset, int(fastaseq.size())), window_size)) {
            if (base != 'N') {
                seq_to_write += base;
...
const string Trailmix::fa2fq(const string & fastaseq, const char & dummyqualscore, const string & tmpdir) {
    ...
    string seq_to_write, qual_to_write;
     ...
     for (int i = 0; i < 101; ++i) {
        Trailmix::write_fq_read(dummyFASTQFile, offset, window_size, fastaseq, dummyqualscore);
        offset += 100;
                                   }
