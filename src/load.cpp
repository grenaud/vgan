...
bool isNonZero(const vector<vector<bool>>& matrix) {
...
string graph_dir = dta->running_trailmix ? dta->graph_dir : dta->hc_graph_dir;
    ...
    std::string mappability_path = getFullPath(graph_dir + "mappability.tsv");
    igzstream myfile;
...
string graph_dir = dta->running_trailmix ? dta->graph_dir : dta->hc_graph_dir;
const string pangenome_map_path = getFullPath(graph_dir+"parsed_pangenome_mapping");
igzstream myfile;
...
string path_name_path = getFullPath(graph_dir+"graph_paths");
igzstream myfile;
...
while (getline(myfile, line))
  {
     ...
     dta->path_names.emplace_back(path);
     p_index++;
  }
...
const string path_support_path = getFullPath(graph_dir+"path_supports");
igzstream myfile;
myfile.open(path_support_path.c_str(), ios::in);
string line;
int index = 0;
