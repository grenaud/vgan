...
int rpvg_main(int argc, char* argv[]) {
    ...
    options.add_options("Vgan")
    ("vgan-temp-dir", "Vgan temporary directory", cxxopts::value<string>())
    ;
    ...
    uint32_t num_threads = option_results["threads"].as<uint32_t>();
    ...
    string vgan_temp_dir = option_results["vgan-temp-dir"].as<string>();
    ...
    bool collapse_haps = (inference_model == "transcripts" && option_results.count("path-info"));
    ...
    if (inference_model == "haplotypes"){
        cerr << "finding alignment paths" << endl;
                                        }
    ...
    //cerr << "Alignment path finder complete, closing istream" << endl;
    ...
    //cerr << "Alignment paths closed" << endl;
    ...
    if(inference_model=="haplotypes"){
    cerr << "Clustered paths" << endl;
                                     }

    collapse_haps = false;

    int original_num_threads = num_threads;
    //num_threads = 1;
    ...
    for (size_t i = 0; i < align_paths_clusters_indices.size(); ++i) {
        ...
        //cerr << "ALIGN PATH CLUSTER: " << i << endl;
    ...
    if (inference_model == "haplotypes") {
        ...
        JointHaplotypeEstimatesWriter joint_haplotype_estimates_writer(option_results["output-prefix"].as<string>(), 1, ploidy, prob_precision);
